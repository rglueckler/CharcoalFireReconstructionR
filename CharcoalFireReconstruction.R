...
# # #
    ...
    # Set working directory (.../CharcoalFireReconstructionR)
setwd()
...
n_agg = 3 # Number of consecutive samples to aggrgate if use_data_aggregation = TRUE
...
input <- data.frame(depth = data$depth_top, 
    ...
    # Optional data aggregation, used when resolution is much smaller than age uncertainty
if (use_data_aggregation == TRUE) {
agg_i <- list(rep(1:nrow(input), each = n_agg)[1:nrow(input)])
input_agg <- data.frame(depth = aggregate(input$depth, agg_i, "mean")$x,
                        age = aggregate(input$age, agg_i, "mean")$x,
                        age_error = aggregate(input$age_error, agg_i, "mean")$x,
                        proxy = aggregate(input$proxy, agg_i, "mean")$x,
                        proxy_error = aggregate(input$proxy_error, agg_i, "mean")$x)
input <- input_agg
}
...
run_sett <- rbind(c("--- Classic CHAR inputs ---",  "\n",
                    ...
                    "(Mean) threshold:", mean(treshold), "\n",
                    "\n",
                    "--- Robust CHAR inputs ---",  "\n",
                    "Aggregated data?", use_data_aggregation, "\n",
                    "If aggregation was used, number of aggregated samples:", n_agg, "\n",
