...
# The robust approach (robust CHAR) follows a similar overall procedure, but also includes a Monte Carlo-based method to incorporate age and 
# proxy uncertainties into the resulting timeseries. 
...
library(paleofire)
    ...
    # Load functions
source('./Function_Robust CHAR_Dietze et al 2019.R')
source('./Function_SNI_Kelly et al 2011.R')
...
plot(1950-x, peak, xaxt = "n", xlab = "", ylab = "", las = 1, type="h", lend = 1, lwd = 3, 
     col = ifelse(peak < treshold,'darkgrey','grey30'), cex.axis = 2, yaxt = "n")
...
par(mar = c(0,5,0,5))
plot(1950-x, SNI$SNI, type="l", lwd = 2, ylab = "", las = 1, xlab = "", xaxt = "n", cex.axis = 1.5, yaxt = "n")
...
plot(1950-x, y, xlim = range(1950-data$age_top), xlab = "", ylim = range(CHAR$accI), ylab = "", las = 1, type = "l",lty = 1, lwd = 2, 
     col = "black", xaxt = "n", cex.axis = 2, yaxt = "n")
axis(2, cex.axis = 2, las = 1)
...
plot(NA, xlim = range(1950-data$age_top), ylim = range(y_back),
     xlab="", ylab = "", las = 1, xaxt = "n", cex.axis = 2, yaxt = "n")
axis(4, cex.axis = 2, las = 1)
...
plot(NA,  xlim = range(1950-data$age_top), ylim = range(y_peak),
     xlab="Age (yr CE)", ylab = "", las = 1, xaxt = "n", cex.axis = 2)
...
title("Robust CHAR peak component", adj = 0.01, line = -1)
    ...
    # Robust CHAR (sum)
par(mar = c(5,5,0,5))
plot(NA, ylim = c(min(char.uncert$q_25), max(range(char.uncert$q_75))),  xlim = range(1950-data$age_top), xlab="Age (CE)", ylab = "", las = 1, cex.lab = 2, cex.axis = 2, yaxt = "n")
axis(4, cex.axis = 2, las = 1)
